...
#fit and transform using pipeline Model
...
#Use Cross Validation to find predictions
numFolds = 3
MaxIter = 5
...
with open('/dbfs/FileStore/LogisticRegressionResults.txt', 'w') as f:  
  print("Training model with Logistic Regression algorithm", file=f)
...
with open('/dbfs/FileStore/LogisticRegressionResults.txt', 'a+') as f:  
  print("Classification accuracy of Logistic Regression: ", accuracy, file=f)
...
with open('/dbfs/FileStore/LogisticRegressionResults.txt', 'r') as f:
  print(f.readlines())
